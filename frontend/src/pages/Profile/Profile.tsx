...
export function Profile() {
  ...
  const handleSubmit = ({
    ...
    if (senha !== '') {
      ...
      if (senha.length < MINIMUM_PASS_SIZE) {
        setErrorSchema((err) => ({
          ...err,
          senha: `Senha deve ter mais de ${MINIMUM_PASS_SIZE} caracteres`,
        }));
        return;
      }
    }
  ...
  return (
    <>
      <Form<UserEdit> onSubmit={handleSubmit} onBlur={resetErrors}>
        <FormGroup>
          <Input name='nome' label='Nome' type='text' value={userData?.nome} />
        </FormGroup>
